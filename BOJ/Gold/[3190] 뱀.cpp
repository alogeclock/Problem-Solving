...
int main() {
    ...
    ll N, K; cin >> N >> K; // N: board size, K: number of apple
    for (int i = 1; i <= K; i++) {
        int r, c; cin >> r >> c;
        board[r][c] = 2; // 0: blank, 1: snake, 2: apple
    }

    ll L; cin >> L; // L: number of direction changes
    vector<pll> dv;
    for (int i = 0; i < L; i++) {
        ll x; 
        char c; 
        ...
        if (c == 'L') dv.push_back({x, 0});
        else dv.push_back({x, 1});
    }
    ...
    while (1) {
        ...
        if (nr > N || nc > N || nr <= 0 || nc <= 0 || board[nr][nc] == 1) break;
        ...
        if (dv[dv_offset].first == timestamp) {
            if (dv[dv_offset].second == 0) dir = (dir - 1) % 4;
            else dir = (dir + 1) % 4;
            if (dir < 0) dir += 4;
            dv_offset++;
        }
